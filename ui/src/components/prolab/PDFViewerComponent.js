...
const PDFViewerComponent = ({file}) => {
    ...
    const page = () => {
        return (
            <b style={{fontSize: '18px'}}>{`${LocUtils.locFromStoreWithDefault(
                'Page',
                'Strona'
            )}  ${pageNumber}/${numPages}`}</b>
        );
    };
    ...
    return (
        <div className='container-fluid justify-content-center'>
            <TransformWrapper initialScale={1} minScale={0.5} maxScale={5} wheel={{step: 0.1}}>
                {() => (
                    <TransformComponent>
                        <Document renderMode='canvas' file={pdfArrayBuffer} onLoadSuccess={onDocumentLoadSuccess}>
                            <Page
            ...
            {pdfMounted && (
                <div>
                    <p className='justify-content-center d-flex'>{page()}</p>
                    <div className='float-right'>
                        <ShortcutButton
                            ...
                            label={LocUtils.locFromStoreWithDefault('Next', 'Nastepna')}
                        />
                    </div>
                    <div>
                        <ShortcutButton
                            ...
                            className={`grid-button-panel normal mt-1 mb-1 mr-1`}
                            handleClick={() => setPageNumber(pageNumber - 1)}
                            label={LocUtils.locFromStoreWithDefault('Prev', 'Poprzednia')}
                        />
                    </div>
                </div>
            )}

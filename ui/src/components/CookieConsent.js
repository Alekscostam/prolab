...
class CookieConsent extends Component {
	...
	render() {
		...
		buttonsToRender.push(
			<a
				tabIndex='0'
				className={`p-button p-component p-button-text-only ${buttonClasses}`}
				style={myButtonStyle}
				href={'/#/cookies-page'}
				id={`actionLink-Więcej`}
				title={`Więcej`}
				key={`actionLink-Więcej`}>
				<span className={`p-button-text p-c`}>{'Więcej'}</span>
			</a>
		);
		buttonsToRender.push(
			<a
				...
				<span className={`p-button-text p-c`}>{buttonText}</span>
			</a>
		);
